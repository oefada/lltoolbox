...
class MerchandisingController extends AppController
{
        ...
        if (isset($_POST) && count($_POST) > 0) {
            if (isset($_POST['welcomeSlide'])) {
                ...
                if (isset($_POST['linkUrl'])) {
                    ...
                    if (is_array(
                            $clientArr
                        ) && isset($clientArr['Client']) && isset($clientArr['Client']['clientId'])
                    ) {
    ...
    public function hometabs()
    {
                ...
                if (isset($_POST['tabName']) && is_array($_POST['tabName'])) {
                    ...
                    if (count($tabData) > 0) {
                        ...
                        foreach ($tabData as $xTabKey => &$xTabValue) {
                            ...
                            $xEventQuery = $this->MerchDataGroup->query(
                                'SELECT lltEventId FROM lltEvent WHERE lltEventGroupId=12 AND eventName LIKE ?',
                                array($xTabPrefix . '%')
                            );
                            $xlltEventId = null;
                            ...
                            $this->MerchDataGroup->query(
                                'UPDATE lltEvent SET eventName = ?,lltEventGroupId = ? WHERE lltEventId = ?',
                                array(
                                    $xTabName,
                                    12,
                                    $xlltEventId,
                            ...
                            $xEventQuery = $this->MerchDataGroup->query(
                                'SELECT * FROM lltEvent WHERE lltEventGroupId=12 AND eventName LIKE ?',
                                array($xTabPrefix . '%')
                            );
                            $xlltEventId = $xEventQuery[0]['lltEvent']['lltEventId'];
        ...
        if (isset($_GET['t'])) {
            ...
            if (isset($tabs[$tabName])) {
                $others = $this->getOtherScheduled(
                    $homepageTabsDataType['MerchDataType']['id'],
                    $tabs[$tabName]['merchDataGroupId']
                );
                $this->set('others', $others);
    ...
    public function fauctionld()
    {
        ...
        $futureDates = array();
        foreach ($this->MerchDataEntries->find(
                     'all',
                     array(
                         'recursive' => 0,
                         'conditions' => array('merchDataTypeId' => $merchDataTypeId, 'startDate >= DATE(NOW())')
                     )
                 ) as $fds) {
        ...
        $lastDate = $this->MerchDataEntries->find(
            'first',
            array(
                'recursive' => 0,
        ...
        if (isset($this->params['url']['data']['scheduleDate'])) {
            ...
            if (preg_match(
                '/^20[0-9]{2}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[0-1])$/',
                trim($this->params['url']['data']['scheduleDate'])
            )
            ) {
    ...
    private function getOtherScheduled($merchDataTypeId, $merchDataGroupId = null)
    {
        ...
        // current
        $current = $this->MerchDataEntries->find(
            'first',
            Array(
                'recursive' => -1,
                'conditions' => Array(
        ...
        // next
        $next = $this->MerchDataEntries->find(
            'first',
            Array(
                'recursive' => -1,
                'conditions' => Array(
    ...
    private function setLinkUrl(&$linkUrl)
    {
        ...
        if (!empty($linkUrl)) {
            if (($urlStr = str_replace('http://www.luxurylink.com/', '/', $linkUrl)) || ($urlStr = str_replace(
                    'www.luxurylink.com/',
                    '/',
                    $linkUrl
