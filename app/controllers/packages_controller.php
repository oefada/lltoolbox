...
class PackagesController extends AppController
{
	...
	function summary($clientId, $packageId)
	{
		...
		if ($roomNights = $this->LoaItem->getRoomNights($packageId, $isMultiClientPackage)) {
			if (count($roomNights[0]['LoaItems'][0]['LoaItemRate']) > 1) {
				...
				$dailyRatesMap = array('w0' => 'Su', 'w1' => 'M', 'w2' => 'T', 'w3' => 'W', 'w4' => 'Th', 'w5' => 'F', 'w6' => 'S');
			...
			if ($_POST['isAjax'] == 'true') {
				...
				if ($this->Package->saveAll($this->data)) {
					...
					if (empty($package['Package']['packageId'])) {
						...
						$relData = array('packageId' => $packageId, 'clientId' => $clientId, 'loaId' => $package['Package']['loaId'], 'numNights' => $package['Package']['numNights'], 'percentOfRevenue' => $percentOfRevenue);
	...
	{
		$fields = array('siteId' => 'Site', 'loaId' => 'Loa', 'isBarter' => 'Barter/Remit', 'packageStatusId' => 'Status', 'packageName' => 'Working Name', 'numGuests' => 'Max Num Guests', 'minGuests' => 'Min Num Guests', 'maxAdults' => 'Max Num Adults', 'numNights' => 'Total Nights', 'currencyId' => 'Currency', 'disclaimer' => 'Disclaimer');
		$errors = array();
		...
		if (!empty($this->data)) {
			...
			if (isset($this->data['NewLoaItem'])) {
				...
				foreach ($this->data['NewLoaItem'] as $newRoom) {
					...
					$data = array('loaItemTypeId' => 1, 'loaId' => $package['Loa']['loaId'], 'itemName' => $newRoom['itemName'], 'currencyId' => $packageCurrencyId);
					$this->LoaItem->create();
							...
							foreach ($ratePeriods as $ratePeriod) {
								...
								if (isset($ratePeriod['LoaItemRate'])) {
									foreach ($ratePeriod['LoaItemRate'] as $rate) {
										...
										$rateRel = array('LoaItemRatePackageRel' => array('loaItemRateId' => $rateId, 'packageId' => $packageId, 'numNights' => $numNights));
									...
									$this->LoaItem->LoaItemRatePeriod->LoaItemRate->save($newRate);
									$rateId = $this->LoaItem->LoaItemRatePeriod->LoaItemRate->getLastInsertId();
									$rateRel = array('LoaItemRatePackageRel' => array('loaItemRateId' => $rateId, 'packageId' => $packageId, 'numNights' => $this->Package->field('numNights', array('packageId' => $packageId))));
									$this->Package->LoaItemRatePackageRel->create();
									$this->Package->LoaItemRatePackageRel->save($rateRel);
								}
					...
					$rel = array('packageId' => $packageId, 'loaItemId' => $loaItemId, 'quantity' => $newRoom['quantity']);
					$this->Package->PackageLoaItemRel->create();
					$this->Package->PackageLoaItemRel->save($rel);
					$packageLoaItemIds[] = array('loaItemId' => $loaItemId, 'isNew' => 1, 'isExistingAddition' => 0, 'quantity' => $newRoom['quantity']);
			...
			foreach ($this->data['LoaItem'] as $loaItemId => $room) {
				...
				if (isset($room['checked'])) {
					...
					$items += $room['quantity'];
					$packageLoaItemIds[] = array('loaItemId' => $loaItemId, 'isNew' => 0, 'isMaster' => $isMaster, 'quantity' => $room['quantity']);
				} else {
			...
			foreach ($packageLoaItemIds as $packageLoaItem) {
				...
				//debug($loaItem);
				//die();
				if ($packageLoaItemRel = $this->Package->PackageLoaItemRel->find('first', array('conditions' => array('PackageLoaItemRel.packageId' => $packageId, 'PackageLoaItemRel.loaItemId' => $loaItemId)))) {
					...
					$rel = array('packageId' => $packageId, 'loaItemId' => $loaItemId, 'quantity' => $packageLoaItem['quantity']);
					$this->Package->PackageLoaItemRel->create();
					$this->Package->PackageLoaItemRel->save($rel);
				}
				if ($ratePeriods = $this->LoaItem->LoaItemRatePeriod->getRatePeriods($loaItemId, $packageId)) {
					foreach ($ratePeriods as $ratePeriod) {
						if (isset($ratePeriod['LoaItemRate']) && !empty($ratePeriod['LoaItemRate'])) {
							foreach ($ratePeriod['LoaItemRate'] as $rate) {
								if (!$loaItemRatePackageRel = $this->Package->LoaItemRatePackageRel->find('all', array('conditions' => array('LoaItemRatePackageRel.packageId' => $packageId, 'LoaItemRatePackageRel.loaItemRateId' => $rate['LoaItemRate']['loaItemRateId'])))) {
									$rel = array('packageId' => $packageId, 'loaItemRateId' => $rate['LoaItemRate']['loaItemRateId'], 'numNights' => $this->Package->field('numNights', array('Package.packageId' => $packageId)));
									$this->Package->LoaItemRatePackageRel->create();
							...
							$this->LoaItem->LoaItemRatePeriod->LoaItemRate->save($newRate);
							$rateId = $this->LoaItem->LoaItemRatePeriod->LoaItemRate->getLastInsertID();
							$rel = array('packageId' => $packageId, 'loaItemRateId' => $rateId, 'numNights' => $this->Package->field('numNights', array('Package.packageId' => $packageId)));
							$this->Package->LoaItemRatePackageRel->create();
							$this->Package->LoaItemRatePackageRel->save($rel);
						}
					}
				...
				if (!empty($loaItem['AddInclusion'])) {
					foreach ($loaItem['AddInclusion'] as $loaItemId => $inclusion) {
						if (isset($inclusion['perNight'])) {
							if ($isMultiClientPackage) {
								$clientLoaId = $this->Package->PackageLoaItemRel->LoaItem->field('loaId', array('LoaItem.loaItemId' => $loaItemId));
								$clientRoomId = $this->Package->PackageLoaItemRel->LoaItem->getClientRoomId($clientLoaId, $packageId);
								$quantity = $this->Package->LoaItemRatePackageRel->getNumNights($clientRoomId, $packageId);
							} else {
						...
						$newInclusion = array('PackageLoaItemRel' => array('loaItemId' => $loaItemId, 'packageId' => $packageId, 'quantity' => $quantity));
						$this->Package->PackageLoaItemRel->create();
...
//define('LOGIT', true);
			...
			if (!$isMultiClientPackage) {
				...
				foreach ($inc as $i) {
					if (isset($i['LoaItem']['PackagedItems'])) {
						...
						$inclusions[] = array('Group' => 1, 'LoaItem' => $group_items, 'PackageLoaItemRel' => array('packageLoaItemRelId' => $i['PackageLoaItemRel']['packageLoaItemRelId']));
					}
				}
	...
	{
		$range = array('LoaItemDate' => array('loaItemRatePeriodId' => $_GET['ratePeriodId'], 'loaItemDateId' => '', 'startDate' => '', 'endDate' => ''));
	...
	function getRatePeriodsInfo($packageId)
	{
		...
		if (!empty($ratePeriods)) {
			foreach ($ratePeriods as $key => &$ratePeriod) {
				...
				$ratePeriods[$key]['auctionPrice'] = (empty($ratePeriod['PricePoint']['percentRetailAuc'])) ? round($total + $totals[$clientName]['inclusionTotal']) : round(($total + $totals[$clientName]['inclusionTotal']) * ($ratePeriod['PricePoint']['percentRetailAuc'] / 100));
				$ratePeriods[$key]['buyNowPrice'] = (empty($ratePeriod['PricePoint']['percentRetailBuyNow'])) ? round($total + $totals[$clientName]['inclusionTotal']) : round(($total + $totals[$clientName]['inclusionTotal']) * ($ratePeriod['PricePoint']['percentRetailBuyNow'] / 100));
