    ...
    public function auction_winner()
    {
        if (!empty($this->data)) {
            ...
            $sql = "
              ...
              LEFT JOIN offerLuxuryLink USING(offerId)
              LEFT JOIN offerFamily USING(offerId)
              LEFT JOIN country AS Country ON (PaymentDetail.ppBillingCountry = Country.countryCode)
            ...
            foreach ($results as $key => $arr) {
                //if($arr['Ticket']['ticketId'] == '239938'){
                   // $ticketAccountingHelper->setTicketAmount($results[$key][0]['revenue']);
                    $ticketAccountingHelper->setTicketAmount(floatval($results[$key]['Ticket']['billingPrice']));
