...
class RefundRequestsController extends AppController
{
    ...
    public function add($ticketId)
    {
        if (!empty($this->data)) {
            ...
            if ($this->data['RefundRequest']['refundOrCOF'] == '') {
                $this->Session->setFlash(
                    __('The Refund Request could not be saved.<br/>Refund / COF Select is required', true)
                );
            } elseif ($this->data['RefundRequest']['cancelFeeLL'] == '') {
                $this->Session->setFlash(
                    __('The Refund Request could not be saved.<br/>LL Cancel Fee is required', true)
                );
            } else {
        ...
        $refundReasons = $this->RefundRequest->RefundReason->find(
            'list',
            array('conditions' => array("inactive" => 0))
        );
        $refundStatuses = $this->RefundRequest->RefundRequestStatus->find('list');
    ...
    {
        $currentUser = $this->LdapAuth->user();
        $access = ($this->hasAccountingAccess(
            ) || $currentUser['LdapUser']['samaccountname'] == 'bjensen' || $currentUser['LdapUser']['samaccountname'] == 'jcross') ? true : false;
        return $access;
    }
    ...
    {
        $currentUser = $this->LdapAuth->user();
        $access = (in_array('Accounting', $currentUser['LdapUser']['groups']) || in_array(
                'Geeks',
