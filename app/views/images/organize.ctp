...
<div class="sitesTab">
	...
	?>
	<div id="<?php echo $site;?>" class="<?php echo ($site == $displayTab) ? ' siteActive' :  ' siteInactive'; ?>">
		<?php __($sites[$site]);?>
	</div>
	<?php endforeach;?>
	<div id="resizer" onclick="return resizeTabClick();" class="siteInactive">
		Resizer <?php echo $html->image('http://showcase.luxurylink.com/clients/toolbox/' . $client['Client']['clientId'] . '?oldProductId=' . $client['Client']['oldProductId'] . '&rand=' . mt_rand(100000, 999999), array(
			'width' => '8px',
			'height' => '8px'
		));
		?>
...
<div id="images-<?php echo $site;?>" class="organize" <?php if ($site != $displayTab): ?> style="display:none"<?php endif;?>>
	<form method="post" id="organizeImages-<?php echo $site;?>">
		...
		<div style="position:relative">
			...
			?>
			<span class="duplicateTo">
				<input type="checkbox" name="data[duplicateTo][<?php echo ($site == 'luxurylink') ? 'family' : 'luxurylink'; ?>]" />
				Copy to <?php echo ($site == 'luxurylink') ? 'Family' : 'Luxury Link';
				?></span>
			<?php endif;?>
			<span class="previewImages"> <?php
			if (in_array('luxurylink', $client['Client']['sites']) && $site == 'luxurylink') {
				echo $html->link('<span>Preview on LuxuryLink</span>', "http://www.luxurylink.com/luxury-hotels/preview.html?clid={$client['Client']['clientId']}&preview=client", array(
					'target' => '_blank',
					'class' => 'button'
				), null, false);
			}
			if (in_array('family', $client['Client']['sites']) && $site == 'family') {
				echo $html->link('<span>Preview on FamilyGetaway</span>', "http://www.familygetaway.com/luxury-hotels/preview.html?clid={$client['Client']['clientId']}&preview=client", array(
					'target' => '_blank',
					'class' => 'button'
				), null, false);
			}
...
<div class="organize slideshow">
	<h3>Slideshow</h3>
	<ul id="sortableSlideshow-<?php echo $site;?>" class="sortableSS"></ul>
</div>
<?php endif;?>
<?php
	break;
...
<div class="organize large">
	...
	<ul id="largeImages-<?php echo $site ?>">
		...
		<li>
			...
			<div class="filename large-filename">
				<?php echo end($fileArr);?>
			</div>
...
<div class="organize thumbnails">
	...
	<ul id="thumbImages-<?php echo $site ?>">
		...
		<li>
			...
			<div class="filename thumb-filename">
				<?php echo end($fileArr);?>
			</div>
...
Sortable.create('sortableSlideshow-<?php echo $site;?>
	', {'overlap':'horizontal',
	'constraint':false,
	'ghosting':true,
	'dropOnEmpty':true,
	'containment':[$('sortableSlideshow-
<?php echo $site;?>'), $('sortableInactive-<?php echo $site;?>
	')]
	}
	);
	Sortable.create('sortableInactive-
<?php echo $site;?>
	', {'overlap':'horizontal',
	'constraint':false,
	'ghosting':true,
...
<?php echo $site;?>'), $('sortableInactive-<?php echo $site;?>
	...
	Event.observe('organizeImages-
<?php echo $site;?>
	', 'submit', function() {
	$('sortableInactive-
<?php echo $site;?>
	').adjacent('
	input.ss').each(function(hiddenField) {
...
<?php echo $site;?>
	...
	window.location.replace('http://showcase.luxurylink.com/clients/view/
<?php echo $client['Client']['clientId'] . '/oldProductId:' . $client['Client']['oldProductId'];?>
	');
	}
	return false;
	}
...
var sitesArr = new Array();<?php foreach ($sites as $site => $siteName): ?><?php echo "sitesArr.push('" . $site . "');";?><?php endforeach;?>
	sitesArr.each(function(siteTab) {
		$('images-' + siteTab).toggle();
	});
	$$('div.siteActive').each(function(tab) {
		$(tab).removeClassName('siteActive').addClassName('siteInactive');
	});
