...
<div class="mB mT">
	...
	foreach ($packageRatePeriods['IncludedItems'] as $a => $b) {
		...
		foreach($b['PackageRatePeriod'] as $itemRatePeriod) {
			...
			echo '<td style="text-align: right;">' . $number->currency($itemRatePeriod['ratePeriodPrice']*$itemRatePeriod['quantity']+$itemRatePeriod['ratePeriodPrice']*$b['feePercent']/100, $currencyCode). '</td>';
		}
		echo '</tr>';
		echo "<tr{$class}>";
		foreach($b['PackageRatePeriod'] as $itemRatePeriod) {
