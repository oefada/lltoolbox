...
<?= $form->input(
    'arrivalDate',
    array('label' => 'Arrival Date', 'empty' => '-- ', 'minYear' => date('Y') - 2, 'maxYear' => date('Y') + 2)
); ?>
...
<div style="width: 50%; background-color: #dadada; border: 1px solid #ccc; margin: 0 0 20px 0;">
    ...
    <?= $form->input(
        'refundHandlingFeeFlag',
        array('label' => 'Refund Handling', 'options' => array('0' => 'No', '1' => 'Yes'), 'class' => 'refund-update')
    ); ?>
</div>
...
<?= $form->input(
    'paymentDetailId',
    array('label' => 'CC to Credit', 'options' => $refundInfo['creditCards'], 'empty' => '-- ')
); ?>
<?= $form->input('notes'); ?>
