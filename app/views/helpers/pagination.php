...
class PaginationHelper extends AppHelper
{
    ...
    function paginate( $link, $page, $total, $options = array(), $show = 9, $skip='&hellip;' )
    {
        ...
        */
        ...
        $skip = '<span class="current">'.$skip.'</span>';    // add spans around skip text
        $out = "\n";
        ...
        // Previous link
        $out .= ( ($page-1) > 0 )
                    ? $this->{$obj}->link( '&lt;&lt; Prev', $link.($page-1), array_merge(array('title'=>'View the previous index page', 'class'=>'nextprev'), $options), array(), false, false ) 
        ...
        $out .= ( $page == 1 )
                    ...
                    : '<span>'.$this->{$obj}->link( '1', $link.'1', array_merge(array('title'=>'View index page 1'), $options) ).'</span>';
        $out .= "\n";
        ...
        for ( $i=0; $i<( ($total<$show+2) ? $total-2 : $show ); $i++ )
        {
            ...
            if ( $i == 0 )
            {
                $out .= ( $start == 2 ) 
                            ? ( $page == 2 )
                                ...
                                : '<span>'.$this->{$obj}->link( '2', $link.'2', array_merge(array('title'=>'View index page 2'), $options) ).'</span>'
                            : $skip;
            }
            ...
            elseif ( $i == ($show-1) )
            {
                $out .= ( $start >= ($total-$show) ) 
                            ? ( $page == ($total-1) )
                                ...
                                : '<span>'.$this->{$obj}->link( ($total-1), $link.($total-1), array_merge(array('title'=>'View index page '.($total-1)), $options) ).'</span>'
                            : $skip;
            }
            ...
            else 
            {
                $out .= ( $page == ($start+$i) )
                            ...
                            : '<span>'.$this->{$obj}->link( ($start+$i), $link.($start+$i), array_merge(array('title'=>'View index page '.($start+$i)), $options) ).'</span>';
            }
        ...
        if ( $total > 1 )
        {
            $out .= ( $page == $total )
                        ...
                        : '<span>'.$this->{$obj}->link( $total, $link.$total, array_merge(array('title'=>'View index page '.$total), $options) ).'</span>';
            $out .= "\n";
        }

        // Next link
        $out .= ( ($page+1) <= $total )
                    ? $this->{$obj}->link( 'Next &gt;&gt;', $link.($page+1), array_merge(array('title'=>'View the next index page', 'class'=>'nextprev'), $options), array(), false, false )
                    : '<div class="disabled">Next &gt;&gt;</div>';
        $out .= "\n";

        // Return
        return '<div class="paging-container"><div class="paging">'.$out.'</div></div>';
    }
