...
<div class="users form">
    ...
    <fieldset>
        ...
        <div
            style="font-size: 14px; color: #990000; font-weight: bold; margin-left: 174px; padding: 15px 15px 13px 20px; background-color: #fdcab9; width: 280px; border-radius: 4px;">
            Credit on File Balance:</strong> <a
                href="/credit_trackings/view/<?= $this->data['User']['userId'] ?>">$<? if (!isset($this->data['CreditTracking'])): ?>0.00<? else: ?><?= number_format(
                    $this->data['CreditTracking']['balance'],
                    2
                ) ?><? endif; ?></a></div>
            ...
            <div class="collapsibleContent">
                <?php if (!empty($userSiteExtended['username'])): ?>
                    <div class="input text"><label>Username</label>
                        <strong><?php echo $userSiteExtended['username']; ?></strong></div>
                    <div class="input text"><label>Registration
                            Date</label> <?= isset($userSiteExtended['registrationDatetime']) ? $userSiteExtended['registrationDatetime'] : 'N/A'; ?>
                    </div>
                    <div class="input text"><label>Last Login</label> <?php echo $userSiteExtended['lastLogin']; ?>
                    </div>
                    <div class="input text"><label>&nbsp;</label><?php echo $html->link(
                            'Reset Password',
                            '/users/resetPassword/' . $userSiteExtended['userSiteExtendedId'],
                            array(
                ...
                foreach ($user['UserPaymentSetting'] as $userPaymentSetting):
                    ...
                    <tr<?php echo $class; ?>>
                        ...
                        <td class="actions">
                            ...
                            <?php echo $html->link(
                                __('Delete', true),
                                '/users/' . $user['User']['userId'] . '/user_payment_settings/delete/' . $userPaymentSetting['userPaymentSettingId']
                            ); ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
        ...
        <div class="actions">
            <ul>
                <li><?php echo $html->link(
                        'Add User Payment Setting',
                        '/users/' . $user['User']['userId'] . '/user_payment_settings/add',
                        array(
    ...
    <div class="collapsibleContent related">
                ...
                foreach ($user['Contest'] as $contest):
                    ...
                    <tr<?php echo $class; ?>>
                        ...
                        <td class="actions">
                            <?php echo $html->link(
                                __('View', true),
                                array('controller' => 'contests', 'action' => 'view', $contest['contestId'])
                            ); ?>
                            <?php echo $html->link(
                                __('Edit', true),
                                array('controller' => 'contests', 'action' => 'edit', $contest['contestId'])
                            ); ?>
                            <?php echo $html->link(
                                __('Delete', true),
                                array('controller' => 'contests', 'action' => 'delete', $contest['contestId']),
                                null,
                                sprintf(__('Are you sure you want to delete # %s?', true), $contest['contestId'])
                            ); ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
        ...
        <div class="actions">
            <ul>
                <li><?php echo $html->link(
                        __('New Contest', true),
                        array('controller' => 'contests', 'action' => 'add')
                    ); ?> </li>
            </ul>
        </div>
    </div>
