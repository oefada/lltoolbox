...
<?php echo $form->create('Loa');?>
	...
	<?php
		...
		if (in_array($userDetails['username'], array('jlagraff','fmartinez','alee','dpen','kferson','kgathany','rfriedman'))) {
			$disabled = false;
		} else {
			$disabled = true;
		}
		...
		$disable_advanced_edit = (in_array($userDetails['username'], array('alee','rhastings','kferson'))) ? false : true;

		// for editing membershipPackagesRemaining
		$disable_mp = (in_array($userDetails['username'], array('alee','rhastings','kferson'))) ? false : true;
