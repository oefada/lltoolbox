...
//                var_dump($roomNightData[$thisRatePeriodIdKey]);
                ...
                $validity_record_height = $numFees + 2;
                    ...
                    $lp_row_offset = $validity_data_row_start + count($lowPrice) * ($validity_record_height + $linesBeforeNextRecord);
                        ...
                        $as->getCell($feeColumn . $lp_row_offset)->setValue($lp['flexPricePerNight']);
...
//            var_dump($key);
        ...
        $i= 1;
         while ($lp = array_pop($lowPrice2)) {
            ...
            $as->getCell('B' . ($lp_row_offset +6))->setValue('Percentage of Retail:');
             ...
             **/
             ...
             $flexPercentOfRetail = '=('.floatval($lp['flexPricePerNight']).'/'.$retailValue.')';
             ...
             $totalWithInclusions = '=('.$validityTotalCell.'+ '.$this->cellTotalInclusions.')';
             ...
             $totalAucPercentOfRetail = '=ROUND(('.$cellAucPercentOfRetail.'* '.$cellTotalWithInclusions.'),2)';
             $as->getCell('D'. ($lp_row_offset+1))->setValue($totalAucPercentOfRetail);

             $cellBuyNowPercentOfRetail = 'C'.($lp_row_offset+4);
             $totalBuyNowPercentOfRetail = '=ROUND(('.$cellBuyNowPercentOfRetail.'* '.$cellTotalWithInclusions.'),2)';
             $as->getCell('D'. ($lp_row_offset+3))->setValue($totalBuyNowPercentOfRetail);

             $cellFlexPercentOfRetail = 'C'.($lp_row_offset+6);
             $totalFlexPercentOfRetail = '=ROUND(('.$cellFlexPercentOfRetail.'* '.$cellTotalWithInclusions.'),2)';
...
//            $as->getCell('B' . ($lp_row_offset + 12))->setValue('Fees');
    ...
    {
        var_dump('add to current offset: '.$n);

        if (isset($n)) {
            $this->current_row_offset = $this->current_row_offset + $n;
        }
        var_dump('current offset: '.$this->current_row_offset);
    }
