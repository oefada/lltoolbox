 ...
 * vgarcia: edited to conform to our application's naming convention
 *
 * @return resource Result resource identifier
 */
	function query() {
		...
		if (count($args) == 1) {
			...
			if (strpos(strtolower($args[0]), 'findby') === 0) {
				$all  = false;
				$field = Inflector::variable(preg_replace('/findBy/i', '', $args[0])); //vgarcia: changed inflector from underscore to variable
			} else {
				$all  = true;
				$field = Inflector::variable(preg_replace('/findAllBy/i', '', $args[0])); //vgarcia: changed inflector from underscore to variable
			}
